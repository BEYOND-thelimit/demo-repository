...
# 2. 각자 해야할 일
## 2.1 Git 관점
사용법은 크게 2가지로 나뉩니다. 설명을 읽고 본인에게 맞는 사용법을 정하시면 됩니다. 우리는 다양한 branch 전략 중에서 Git-flow 브랜치 전략을 사용하려 합니다.
1. **git 방식**
   - Git flow 전략이 이해가 잘 안된다 or 아예 모른다.
   - Git flow 전략이 무엇인지는 알지만 git-flow를 사용해본적이 없거나 모른다.
...
2. **git-flow 방식**:
   - 내가 Git flow 전략이 어떤건지 이해하고 있으며 git-flow까지 사용할 줄 안다.
...
### 3.4 커밋규칙
Commit Message를 잘 작성해 주세요. 자주 쓰는 키워드는 *기울임체*하였습니다.
